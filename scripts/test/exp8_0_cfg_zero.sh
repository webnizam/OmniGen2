...
experiments=(
ominigenv2_ps512_bs128_ft_edit
)
...
do
    for((j=0;j<${#zero_init_step[@]};j++))
    do
        for((k=0;k<${#shift[@]};k++))
        do
            accelerate launch \
            --main_process_port 40800 \
            --num_machines 1 \
            --mixed_precision no \
            --dynamo_backend no \
            ...
            --zero_init_steps ${zero_init_step[j]} \
            --visualize_input_image \
            --dynamic_size \
            --visualize_progress \
            --result_dir experiments/${experiments[i]}/results_${step}_gs4.0_shift${shift[k]}_cfg_zero_star_zero_init_steps${zero_init_step[j]}
        done
    done
done